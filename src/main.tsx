...
function Donate({lang}:{lang:Lang}) { const t=copy[lang]; const [amount,setAmount]=useState(1000); const [processing,setProcessing]=useState(false); const [success,setSuccess]=useState(false); const [error,setError]=useState<string|null>(null); const [confirmation,setConfirmation]=useState<PaymentConfirmation|null>(null);
 return <main className="page donate-page"><section className="info-hero"><div className="wrap"><p className="eyebrow light">{lang==='mr'?'आपले योगदान, आपला अभिमान':'Your contribution, a shared legacy'}</p><h1>{t.formTitle}</h1><p className="info-intro">{lang==='mr'?'ही देणगी सुरक्षित पेमेंट गेटवेद्वारे पूर्ण केली जाईल.':'You will be securely redirected to an approved payment gateway.'}</p><div className="info-hero-rule" aria-hidden="true"/></div></section>{success?<div className="success card"><span>✓</span><h2>{lang==='mr'?'देणगी पूर्ण झाली':'Donation successful'}</h2><p>{lang==='mr'?'पावती लवकरच उपलब्ध होईल.':'Your receipt will be available once the payment is verified.'}</p><>{confirmation && <DonationDetails lang={lang} confirmation={confirmation}/>}</><ShareCampaign lang={lang}/><button onClick={()=>setSuccess(false)}>{t.payment} →</button></div>:<form name="donationForm" className="donation-form card" onSubmit={async (e)=>{
    e.preventDefault();
    setError(null);
    setProcessing(true);
    try{
...
function App(){const [lang,setLang]=useState<Lang>((localStorage.getItem('lang') as Lang)||'mr');const change=(l:Lang)=>{localStorage.setItem('lang',l);setLang(l)};return <><Header lang={lang} setLang={change}/><div id="page-content" tabIndex={-1}><React.Suspense fallback={<div className="section-loading" role="status">Loading...</div>}><Routes><Route path="/" element={<Home lang={lang}/>}/><Route path="/donate" element={<Donate lang={lang}/>}/><Route path="/project" element={<InformationPage lang={lang} kind="project"/>}/><Route path="/progress" element={<InformationPage lang={lang} kind="progress"/>}/><Route path="/news" element={<InformationPage lang={lang} kind="news"/>}/><Route path="/transparency" element={<Transparency lang={lang}/>}/><Route path="/about" element={<InformationPage lang={lang} kind="about"/>}/><Route path="/activities" element={<Simple lang={lang} kind="activities"/>}/><Route path="*" element={<Home lang={lang}/>}/></Routes></React.Suspense></div><Footer/><MobileDonationBar lang={lang}/><AppExperience lang={lang}/></>};
